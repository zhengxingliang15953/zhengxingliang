  ...
  <div id="book">
    <Modal title="添加图书" v-model="modal1" :mask-closable="false" footer-hide>
      <Form ref="formValidate" :model="formData" :label-width="60">
        <Row type="flex">
          <Col :lg="12">
            <FormItem label="题名" prop="name">
              <Input v-model="formData.name" placeholder="请输入题名"></Input>
            </FormItem>
          </Col>
          <Col :lg="12">
          ...
          <Col :lg="12">
            <FormItem label="日期">
              <Select v-model="formData.date" placeholder="请选择日期">
                <Option :value="item" v-for="(item,index) in 2020" :key="index" v-show="item>1989">{{item}}</Option>
              </Select>
            </FormItem>
          </Col>
          <Col :lg="12">
            <FormItem label="数量">
              <el-input-number v-model="formData.number" :min="1" :max="10" label="描述文字" size="small"></el-input-number>
            </FormItem>
          </Col>
        </Row>
      ...
      <Col :lg="19">
        <p>
          ...
          <span style="color:#8B3A3A;">
            ...
            >馆藏副本{{item.number}},已预约{{item.appNumber}},已借阅{{item.readNumber}}</Poptip>
          </span>
        </p>
        <p>
          ...
          <Button type="error" style="margin-left:4px">删除</Button>
        </p>
        <p>
...
<script>
import { getAllBook ,getAddBook} from "../../api";
import {pressData} from '../../components/press.js'
export default {
  name: "book",
  data() {
    return {
  ...
  methods: {
    ...
    addSubmit(){//添加图书提交
      if(this.formData.isbn==''||this.formData.name==''||this.formData.author==''||this.formData.address==''){
        this.$message.warning('信息请填写完整');
      }else{
        getAddBook(this.formData).then((data)=>{
