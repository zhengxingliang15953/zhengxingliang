    ...
    <el-table :data="ApplyList" border style="width: 100%;margin-top:10px;">
      ...
      <el-table-column fixed="right" label="操作" style="width:10%">
        <template slot-scope="scope">
          <Button type="success" @click="applySuccess(scope.row.sno)">通过</Button>
          <Button type="error" @click="applyError(scope.row.sno)" style="margin-left:20px;">回拒</Button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    ...
    applySuccess(value){//审核通过
        console.log(value);
    },
    applyError(value){//拒绝审核
