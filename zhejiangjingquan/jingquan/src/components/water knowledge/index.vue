      ...
      <Col :lg="4" :sm="24">
        <div class="navOfInside">
          <div class="boxOfTitleInside">
            ...
            <p >{{$t('waterKnowledge')}}</p>
          </div>
          <ul class="listOfNavInside">
            <li v-for="(item, index) in infolist" :key="index" @click="getinfolist(item.id, $i18n.locale)">
...
export default {
  ...
  mounted () {
    this.getinfolist(this.kid, this.$i18n.locale)
  },
  data () {
    return {
      info: '',
  ...
  methods: {
    async getinfolist (id, lang) {
      this.info = await getknowledge(id, lang);
